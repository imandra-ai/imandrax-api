...
def main():
    ...
    assert len(x1.counter_example.model.src) > 0, "expected a non-empty counter-example"

    x2 = c.test_src(src="fun x -> x = 42 || x <> 42")
    assert x2.WhichOneof("res") == "err", "expected err result"
    ...
    print("8. decompose_full")
    d1 = c.decompose_full(decomp.by_name("f"))
    ...
    x_g = c.eval_src(src="let g x = if x > 10 then x * 2 else x ;;")
    ...
    assert d2.WhichOneof("res") == "artifact", f"compound decompose failed: {d2.errors}"
    assert d2.errors == []
    ...
    print("9. verify_name / instance_name")
    x5 = c.eval_src(src="let good x = f x > x ;; let bad x = f x > x + 5 ;;")
    assert x5.success, f"eval_src failed: {x5.errors}"

    v_ok = c.verify_name(name="good")
    assert v_ok.WhichOneof("res") == "proved", f"expected proved, got {v_ok}"

    v_bad = c.verify_name(name="bad")
    ...
    i_ok = c.instance_name(name="good")
    ...
    decls_str = c.get_decls(["f"], include_str=True)
    assert decls_str.decls[0].str, "expected a string representation"

    missing = c.get_decls(["no_such_decl"])
    assert list(missing.not_found) == ["no_such_decl"]

    print("11. eval_src async_only + get_artifact")
    x6 = c.eval_src(src="theorem async_thm x = f x > x ;;", async_only=True)
    ...
    for _ in range(20):
        kinds = c.list_artifacts(async_task).ok.kinds
        if "po_res" in kinds:
            break
        time.sleep(0.5)
    else:
    ...
    art = c.get_artifact(async_task, kind="po_res")
    print(art)
