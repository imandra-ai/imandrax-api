...
let rec pt_arg_of_arg arg x =
  match arg with
  | Term -> spf "A_term %s" x
  | Seq -> spf "A_seq %s" x
  | Int -> spf "A_int %s" x
  (* | String -> spf "A_str %s" x *)
  | List sub ->
    spf "A_list (List.map (fun x -> %s) %s)" (pt_arg_of_arg sub "x") x

let rec get_arg t arg x =
  match arg with
  | Term ->
    spf "(match %s with A_term t -> t | _x -> raise (Bad_arg (%s, _x)))" x t
  | Seq ->
    spf "(match %s with A_seq g -> g | _x -> raise (Bad_arg (%s, _x)))" x t
  | Int ->
    spf "(match %s with A_int i -> i | _x -> raise (Bad_arg (%s, _x)))" x t
  (* | String -> *)
  (*   spf "(match %s with PT.A_sub s -> s | _x -> raise (Bad_arg (%s, _x)))" x t *)
  | List a ->
    spf
      "(match %s with A_list l -> List.map (fun x -> %s) l | _x -> raise \
       (Bad_arg (%s, _x)))"
      x (get_arg t a "x") t
...
let prelude =
 ...
 (* auto-generated in [gen/mk_proof.ml], do not modify *)

module Cir = Imandrax_cir
module Rir = Imandrax_rir
module RT = Rir.Term
module PT = Imandrax_proof.Rir_proof_term
module Sequent = Rir.Sequent
open Imandrax_proof.Arg
|}
...
let main () =
  ...
  let gen_mk_rule (r : rule) : unit =
    ...
    pf "let %s ~concl:c_ %s : PT.t = %sPT.(apply_rule ~concl:c_ %S [%s])\n"
