...
let rec default_eval_src_req 
  ...
  ?task_filter:((task_filter:string list) = [])
  () : eval_src_req  = {
  session;
  src;
  async_only;
...
let rec default_qcheck_src_req 
  ?session:((session:Session.session option) = None)
  ?src:((src:string) = "")
  ?seed:((seed:int64) = 0L)
  () : qcheck_src_req  = {
...
let rec default_qcheck_name_req 
  ?session:((session:Session.session option) = None)
  ?name:((name:string) = "")
  ?seed:((seed:int64) = 0L)
  () : qcheck_name_req  = {
...
let rec default_test_res_res (): test_res_res = Err

and default_test_res 
  ?res:((res:test_res_res) = Err)
  ?errors:((errors:Error.error list) = [])
  ?task:((task:Task.task option) = None)
  () : test_res  = {
  res;
...
let rec default_decl_with_name 
  ?name:((name:string) = "")
  ?artifact:((artifact:Artmsg.art option) = None)
  ?str:((str:string option) = None)
  () : decl_with_name  = {
  name;
  artifact;
  str;
...
let rec default_get_decls_res 
  ?decls:((decls:decl_with_name list) = [])
  ?not_found:((not_found:string list) = [])
  () : get_decls_res  = {
  decls;
  not_found;
}
...
type test_src_req_mutable = {
  mutable session : Session.session option;
  mutable src : string;
  mutable seed : int64;
}

let default_test_src_req_mutable () : test_src_req_mutable = {
  session = None;
  src = "";
  seed = 0L;
}

type qcheck_src_req_mutable = {
  mutable session : Session.session option;
  mutable src : string;
  mutable seed : int64;
}

let default_qcheck_src_req_mutable () : qcheck_src_req_mutable = {
  session = None;
  src = "";
  seed = 0L;
}

type test_name_req_mutable = {
  mutable session : Session.session option;
  mutable name : string;
  mutable seed : int64;
}

let default_test_name_req_mutable () : test_name_req_mutable = {
  session = None;
  name = "";
  seed = 0L;
}

type qcheck_name_req_mutable = {
  mutable session : Session.session option;
  mutable name : string;
  mutable seed : int64;
}

let default_qcheck_name_req_mutable () : qcheck_name_req_mutable = {
  session = None;
  name = "";
  seed = 0L;
}
...
type test_res_mutable = {
  mutable res : test_res_res;
  mutable errors : Error.error list;
  mutable task : Task.task option;
}

let default_test_res_mutable () : test_res_mutable = {
  res = Err;
  errors = [];
  task = None;
}
...
type get_decls_req_mutable = {
  mutable session : Session.session option;
  mutable name : string list;
  mutable str : bool;
}

let default_get_decls_req_mutable () : get_decls_req_mutable = {
  session = None;
  name = [];
  str = false;
}

type decl_with_name_mutable = {
  mutable name : string;
  mutable artifact : Artmsg.art option;
  mutable str : string option;
}

let default_decl_with_name_mutable () : decl_with_name_mutable = {
  name = "";
  artifact = None;
  str = None;
}

type get_decls_res_mutable = {
  mutable decls : decl_with_name list;
  mutable not_found : string list;
}

let default_get_decls_res_mutable () : get_decls_res_mutable = {
  decls = [];
  not_found = [];
}
...
let rec pp_decompose_req fmt (v:decompose_req) = 
  let pp_i fmt () =
    ...
    Pbrt.Pp.pp_record_field ~first:false "prune" (Pbrt.Pp.pp_option Pbrt.Pp.pp_bool) fmt v.prune;
    ...
    Pbrt.Pp.pp_record_field ~first:false "string_results" (Pbrt.Pp.pp_option Pbrt.Pp.pp_bool) fmt v.string_results;
...
let rec pp_test_ok fmt (v:test_ok) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_unit fmt ()
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
let rec pp_po_res_res fmt (v:po_res_res) =
  ...
  | Test_ok  -> Format.fprintf fmt "Test_ok"
...
let rec pp_test_src_req fmt (v:test_src_req) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "session" (Pbrt.Pp.pp_option Session.pp_session) fmt v.session;
    Pbrt.Pp.pp_record_field ~first:false "src" Pbrt.Pp.pp_string fmt v.src;
    Pbrt.Pp.pp_record_field ~first:false "seed" Pbrt.Pp.pp_int64 fmt v.seed;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
let rec pp_test_name_req fmt (v:test_name_req) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "session" (Pbrt.Pp.pp_option Session.pp_session) fmt v.session;
    Pbrt.Pp.pp_record_field ~first:false "name" Pbrt.Pp.pp_string fmt v.name;
    Pbrt.Pp.pp_record_field ~first:false "seed" Pbrt.Pp.pp_int64 fmt v.seed;
...
let rec pp_test_res_res fmt (v:test_res_res) =
  match v with
  | Err  -> Format.fprintf fmt "Err"
  | Counter_example x -> Format.fprintf fmt "@[<hv2>Counter_example(@,%a)@]" pp_counter_sat x

and pp_test_res fmt (v:test_res) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "res" pp_test_res_res fmt v.res;
    Pbrt.Pp.pp_record_field ~first:false "errors" (Pbrt.Pp.pp_list Error.pp_error) fmt v.errors;
    Pbrt.Pp.pp_record_field ~first:false "task" (Pbrt.Pp.pp_option Task.pp_task) fmt v.task;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
let rec pp_get_decls_req fmt (v:get_decls_req) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "session" (Pbrt.Pp.pp_option Session.pp_session) fmt v.session;
    Pbrt.Pp.pp_record_field ~first:false "name" (Pbrt.Pp.pp_list Pbrt.Pp.pp_string) fmt v.name;
    Pbrt.Pp.pp_record_field ~first:false "str" Pbrt.Pp.pp_bool fmt v.str;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()
...
let rec pp_decl_with_name fmt (v:decl_with_name) = 
  let pp_i fmt () =
    Pbrt.Pp.pp_record_field ~first:true "name" Pbrt.Pp.pp_string fmt v.name;
    Pbrt.Pp.pp_record_field ~first:false "artifact" (Pbrt.Pp.pp_option Artmsg.pp_art) fmt v.artifact;
    Pbrt.Pp.pp_record_field ~first:false "str" (Pbrt.Pp.pp_option Pbrt.Pp.pp_string) fmt v.str;
  in
  Pbrt.Pp.pp_brk pp_i fmt ()

let rec pp_get_decls_res fmt (v:get_decls_res) = 
  let pp_i fmt () =
...
let rec encode_pb_decompose_req (v:decompose_req) encoder = 
  ...
  begin match v.string_results with
  | Some x -> 
    Pbrt.Encoder.bool x encoder;
...
let rec encode_pb_test_ok (v:test_ok) encoder = 
()
...
let rec encode_pb_test_src_req (v:test_src_req) encoder = 
  begin match v.session with
  | Some x -> 
    Pbrt.Encoder.nested Session.encode_pb_session x encoder;
    Pbrt.Encoder.key 1 Pbrt.Bytes encoder; 
...
let rec encode_pb_test_name_req (v:test_name_req) encoder = 
  begin match v.session with
  | Some x -> 
    Pbrt.Encoder.nested Session.encode_pb_session x encoder;
    Pbrt.Encoder.key 1 Pbrt.Bytes encoder; 
...
let rec encode_pb_test_res_res (v:test_res_res) encoder = 
  begin match v with
  | Err ->
    Pbrt.Encoder.key 1 Pbrt.Bytes encoder; 
    Pbrt.Encoder.empty_nested encoder
...
and encode_pb_test_res (v:test_res) encoder = 
  begin match v.res with
  | Err ->
    Pbrt.Encoder.empty_nested encoder;
    Pbrt.Encoder.key 1 Pbrt.Bytes encoder; 
...
let rec encode_pb_get_decls_req (v:get_decls_req) encoder = 
  begin match v.session with
  | Some x -> 
    Pbrt.Encoder.nested Session.encode_pb_session x encoder;
    Pbrt.Encoder.key 1 Pbrt.Bytes encoder; 
...
let rec encode_pb_get_decls_res (v:get_decls_res) encoder = 
  Pbrt.List_util.rev_iter_with (fun x encoder -> 
...
let rec decode_pb_decompose_req d =
  ...
  while !continue__ do
    ...
    | Some (6, Pbrt.Varint) -> begin
      v.prune <- Some (Pbrt.Decoder.bool d);
    end
    | Some (6, pk) -> 
    ...
    | Some (9, Pbrt.Varint) -> begin
      v.string_results <- Some (Pbrt.Decoder.bool d);
    end
    | Some (9, pk) -> 
...
let rec decode_pb_eval_src_req d =
  ...
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None -> (
      v.task_filter <- List.rev v.task_filter;
    ); continue__ := false
    | Some (1, Pbrt.Bytes) -> begin
      v.session <- Some (Session.decode_pb_session (Pbrt.Decoder.nested d));
    end
    ...
    | Some (4, Pbrt.Bytes) -> begin
      v.task_filter <- (Pbrt.Decoder.string d) :: v.task_filter;
    end
    | Some (4, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(eval_src_req), field(4)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({
    session = v.session;
...
let rec decode_pb_test_ok d =
  match Pbrt.Decoder.key d with
  | None -> ();
  | Some (_, pk) -> 
    Pbrt.Decoder.unexpected_payload "Unexpected fields in empty message(test_ok)" pk
...
and decode_pb_po_res d =
  ...
  while !continue__ do
    ...
    | Some (6, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(po_res), field(6)" pk
    | Some (10, Pbrt.Bytes) -> begin
      v.errors <- (Error.decode_pb_error (Pbrt.Decoder.nested d)) :: v.errors;
    end
    | Some (10, pk) -> 
...
let rec decode_pb_test_src_req d =
  let v = default_test_src_req_mutable () in
  let continue__= ref true in
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None -> (
    ...
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(test_src_req), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.src <- Pbrt.Decoder.string d;
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(test_src_req), field(2)" pk
    | Some (3, Pbrt.Varint) -> begin
      v.seed <- Pbrt.Decoder.int64_as_varint d;
    end
    | Some (3, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(test_src_req), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({
    session = v.session;
...
let rec decode_pb_qcheck_src_req d =
  let v = default_qcheck_src_req_mutable () in
  let continue__= ref true in
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None -> (
    ...
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(qcheck_src_req), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.src <- Pbrt.Decoder.string d;
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(qcheck_src_req), field(2)" pk
    | Some (3, Pbrt.Varint) -> begin
      v.seed <- Pbrt.Decoder.int64_as_varint d;
    end
    | Some (3, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(qcheck_src_req), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({
    session = v.session;
...
let rec decode_pb_test_name_req d =
  let v = default_test_name_req_mutable () in
  let continue__= ref true in
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None -> (
    ...
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(test_name_req), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.name <- Pbrt.Decoder.string d;
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(test_name_req), field(2)" pk
    | Some (3, Pbrt.Varint) -> begin
      v.seed <- Pbrt.Decoder.int64_as_varint d;
    end
    | Some (3, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(test_name_req), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({
    session = v.session;
...
let rec decode_pb_qcheck_name_req d =
  let v = default_qcheck_name_req_mutable () in
  let continue__= ref true in
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None -> (
    ...
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(qcheck_name_req), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.name <- Pbrt.Decoder.string d;
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(qcheck_name_req), field(2)" pk
    | Some (3, Pbrt.Varint) -> begin
      v.seed <- Pbrt.Decoder.int64_as_varint d;
    end
    | Some (3, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(qcheck_name_req), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({
    session = v.session;
...
let rec decode_pb_test_res_res d = 
  let rec loop () = 
    let ret:test_res_res = match Pbrt.Decoder.key d with
      | None -> Pbrt.Decoder.malformed_variant "test_res_res"
      | Some (1, _) -> begin 
        Pbrt.Decoder.empty_nested d ;
        (Err : test_res_res)
      end
      | Some (2, _) -> (Counter_example (decode_pb_counter_sat (Pbrt.Decoder.nested d)) : test_res_res) 
      | Some (n, payload_kind) -> (
        Pbrt.Decoder.skip d payload_kind; 
        loop () 
      )
...
and decode_pb_test_res d =
  let v = default_test_res_mutable () in
  let continue__= ref true in
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None -> (
    ...
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(test_res), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.res <- Counter_example (decode_pb_counter_sat (Pbrt.Decoder.nested d));
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(test_res), field(2)" pk
    | Some (10, Pbrt.Bytes) -> begin
      v.errors <- (Error.decode_pb_error (Pbrt.Decoder.nested d)) :: v.errors;
    end
    | Some (10, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(test_res), field(10)" pk
    | Some (11, Pbrt.Bytes) -> begin
      v.task <- Some (Task.decode_pb_task (Pbrt.Decoder.nested d));
    end
    | Some (11, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(test_res), field(11)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({
    res = v.res;
...
let rec decode_pb_get_decls_req d =
  let v = default_get_decls_req_mutable () in
  let continue__= ref true in
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None -> (
    ...
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(get_decls_req), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.name <- (Pbrt.Decoder.string d) :: v.name;
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(get_decls_req), field(2)" pk
    | Some (10, Pbrt.Varint) -> begin
      v.str <- Pbrt.Decoder.bool d;
    end
    | Some (10, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(get_decls_req), field(10)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({
    session = v.session;
...
let rec decode_pb_decl_with_name d =
  let v = default_decl_with_name_mutable () in
  let continue__= ref true in
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None -> (
    ...
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(decl_with_name), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.artifact <- Some (Artmsg.decode_pb_art (Pbrt.Decoder.nested d));
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(decl_with_name), field(2)" pk
    | Some (3, Pbrt.Bytes) -> begin
      v.str <- Some (Pbrt.Decoder.string d);
    end
    | Some (3, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(decl_with_name), field(3)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({
    name = v.name;
...
let rec decode_pb_get_decls_res d =
  let v = default_get_decls_res_mutable () in
  let continue__= ref true in
  while !continue__ do
    match Pbrt.Decoder.key d with
    | None -> (
      v.not_found <- List.rev v.not_found;
      v.decls <- List.rev v.decls;
    ); continue__ := false
    | Some (1, Pbrt.Bytes) -> begin
      v.decls <- (decode_pb_decl_with_name (Pbrt.Decoder.nested d)) :: v.decls;
    end
    | Some (1, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(get_decls_res), field(1)" pk
    | Some (2, Pbrt.Bytes) -> begin
      v.not_found <- (Pbrt.Decoder.string d) :: v.not_found;
    end
    | Some (2, pk) -> 
      Pbrt.Decoder.unexpected_payload "Message(get_decls_res), field(2)" pk
    | Some (_, payload_kind) -> Pbrt.Decoder.skip d payload_kind
  done;
  ({
    decls = v.decls;
...
let rec encode_json_decompose_req (v:decompose_req) = 
  ...
  in
  let assoc = match v.prune with
    | None -> assoc
    | Some v -> ("prune", Pbrt_yojson.make_bool v) :: assoc
  in
  ...
  in
  let assoc = match v.string_results with
    | None -> assoc
    | Some v -> ("stringResults", Pbrt_yojson.make_bool v) :: assoc
  in
...
let rec encode_json_test_ok (v:test_ok) = 
...
let rec encode_json_test_src_req (v:test_src_req) = 
  let assoc = [] in 
  let assoc = match v.session with
    | None -> assoc
    | Some v -> ("session", Session.encode_json_session v) :: assoc
  in
  let assoc = ("src", Pbrt_yojson.make_string v.src) :: assoc in
  let assoc = ("seed", Pbrt_yojson.make_string (Int64.to_string v.seed)) :: assoc in
  `Assoc assoc
...
let rec encode_json_test_name_req (v:test_name_req) = 
  let assoc = [] in 
  let assoc = match v.session with
    | None -> assoc
    | Some v -> ("session", Session.encode_json_session v) :: assoc
...
let rec encode_json_test_res_res (v:test_res_res) = 
  begin match v with
  | Err -> `Assoc [("err", `Null)]
  | Counter_example v -> `Assoc [("counterExample", encode_json_counter_sat v)]
  end

and encode_json_test_res (v:test_res) = 
  let assoc = [] in 
  let assoc = match v.res with
      | Err -> ("err", `Null) :: assoc
      | Counter_example v -> ("counterExample", encode_json_counter_sat v) :: assoc
  in (* match v.res *)
  let assoc =
    let l = v.errors |> List.map Error.encode_json_error in
    ("errors", `List l) :: assoc 
...
let rec encode_json_get_decls_req (v:get_decls_req) = 
  let assoc = [] in 
  let assoc = match v.session with
    | None -> assoc
    | Some v -> ("session", Session.encode_json_session v) :: assoc
  ...
  let assoc =
    ...
    ("name", `List l) :: assoc 
  in
  let assoc = ("str", Pbrt_yojson.make_bool v.str) :: assoc in
  `Assoc assoc
...
let rec encode_json_decl_with_name (v:decl_with_name) = 
  ...
  let assoc = match v.str with
    | None -> assoc
    | Some v -> ("str", Pbrt_yojson.make_string v) :: assoc
  in
  `Assoc assoc

let rec encode_json_get_decls_res (v:get_decls_res) = 
  let assoc = [] in 
  let assoc =
...
let rec decode_json_decompose_req d =
  ...
  List.iter (function 
    ...
    | ("prune", json_value) -> 
      v.prune <- Some (Pbrt_yojson.bool json_value "decompose_req" "prune")
    | ("ctxSimp", json_value) -> 
    ...
    | ("stringResults", json_value) -> 
      v.string_results <- Some (Pbrt_yojson.bool json_value "decompose_req" "string_results")
    | ("timeout", json_value) -> 
...
let rec decode_json_decompose_req_full_by_name d =
  ...
  List.iter (function 
    ...
    | ("prune", json_value) -> 
      v.prune <- Some (Pbrt_yojson.bool json_value "decompose_req_full_by_name" "prune")
    | ("ctxSimp", json_value) -> 
...
let rec decode_json_decompose_req_full d =
  ...
  List.iter (function 
    ...
    | ("stringResults", json_value) -> 
      v.string_results <- Some (Pbrt_yojson.bool json_value "decompose_req_full" "string_results")
    | ("timeout", json_value) -> 
...
let rec decode_json_eval_src_req d =
  ...
  List.iter (function 
    ...
    | ("taskFilter", `List l) -> begin
      v.task_filter <- List.map (function
...
let rec decode_json_test_ok d =
Pbrt_yojson.unit d "test_ok" "empty record"
...
let rec decode_json_test_src_req d =
  let v = default_test_src_req_mutable () in
  let assoc = match d with
    | `Assoc assoc -> assoc
    | _ -> assert(false)
  ...
  List.iter (function 
    | ("session", json_value) -> 
      v.session <- Some ((Session.decode_json_session json_value))
    | ("src", json_value) -> 
      v.src <- Pbrt_yojson.string json_value "test_src_req" "src"
    | ("seed", json_value) -> 
      v.seed <- Pbrt_yojson.int64 json_value "test_src_req" "seed"
...
let rec decode_json_qcheck_src_req d =
  ...
  List.iter (function 
    | ("session", json_value) -> 
      v.session <- Some ((Session.decode_json_session json_value))
    | ("src", json_value) -> 
      v.src <- Pbrt_yojson.string json_value "qcheck_src_req" "src"
    | ("seed", json_value) -> 
      v.seed <- Pbrt_yojson.int64 json_value "qcheck_src_req" "seed"
...
let rec decode_json_test_name_req d =
  let v = default_test_name_req_mutable () in
  let assoc = match d with
    | `Assoc assoc -> assoc
    | _ -> assert(false)
  ...
  List.iter (function 
    | ("session", json_value) -> 
      v.session <- Some ((Session.decode_json_session json_value))
    | ("name", json_value) -> 
      v.name <- Pbrt_yojson.string json_value "test_name_req" "name"
    | ("seed", json_value) -> 
      v.seed <- Pbrt_yojson.int64 json_value "test_name_req" "seed"
...
let rec decode_json_qcheck_name_req d =
  ...
  List.iter (function 
    | ("session", json_value) -> 
      v.session <- Some ((Session.decode_json_session json_value))
    | ("name", json_value) -> 
      v.name <- Pbrt_yojson.string json_value "qcheck_name_req" "name"
    | ("seed", json_value) -> 
      v.seed <- Pbrt_yojson.int64 json_value "qcheck_name_req" "seed"
...
let rec decode_json_test_res_res json =
  let assoc = match json with
    | `Assoc assoc -> assoc
    | _ -> assert(false)
  in
  let rec loop = function
    | [] -> Pbrt_yojson.E.malformed_variant "test_res_res"
    | ("err", _)::_-> (Err : test_res_res)
    | ("counterExample", json_value)::_ -> 
      (Counter_example ((decode_json_counter_sat json_value)) : test_res_res)
    
    | _ :: tl -> loop tl
  in
  loop assoc

and decode_json_test_res d =
  let v = default_test_res_mutable () in
  let assoc = match d with
    | `Assoc assoc -> assoc
    | _ -> assert(false)
  ...
  List.iter (function 
    ...
    | ("counterExample", json_value) -> 
      v.res <- Counter_example ((decode_json_counter_sat json_value))
    | ("errors", `List l) -> begin
      v.errors <- List.map (function
        | json_value -> (Error.decode_json_error json_value)
      ) l;
...
let rec decode_json_get_decls_req d =
  let v = default_get_decls_req_mutable () in
  let assoc = match d with
    | `Assoc assoc -> assoc
    | _ -> assert(false)
  ...
  List.iter (function 
    | ("session", json_value) -> 
      v.session <- Some ((Session.decode_json_session json_value))
    | ("name", `List l) -> begin
      v.name <- List.map (function
        | json_value -> Pbrt_yojson.string json_value "get_decls_req" "name"
      ) l;
    end
    | ("str", json_value) -> 
      v.str <- Pbrt_yojson.bool json_value "get_decls_req" "str"
...
let rec decode_json_decl_with_name d =
  ...
  List.iter (function 
    | ("name", json_value) -> 
      v.name <- Pbrt_yojson.string json_value "decl_with_name" "name"
    | ("artifact", json_value) -> 
      v.artifact <- Some ((Artmsg.decode_json_art json_value))
    | ("str", json_value) -> 
      v.str <- Some (Pbrt_yojson.string json_value "decl_with_name" "str")
...
let rec decode_json_get_decls_res d =
  let v = default_get_decls_res_mutable () in
  let assoc = match d with
    | `Assoc assoc -> assoc
    | _ -> assert(false)
  ...
  List.iter (function 
    | ("decls", `List l) -> begin
      v.decls <- List.map (function
        | json_value -> (decode_json_decl_with_name json_value)
      ) l;
    end
    | ("notFound", `List l) -> begin
      v.not_found <- List.map (function
        | json_value -> Pbrt_yojson.string json_value "get_decls_res" "not_found"
      ) l;
    end
...
module Simple = struct
  ...
  module Client = struct
    ...
    let test_src : (test_src_req, unary, test_res, unary) Client.rpc =
      (Client.mk_rpc 
        ~package:["imandrax";"simple"]
        ~service_name:"Simple" ~rpc_name:"test_src"
        ~req_mode:Client.Unary
        ~res_mode:Client.Unary
        ~encode_json_req:encode_json_test_src_req
        ~encode_pb_req:encode_pb_test_src_req
        ~decode_json_res:decode_json_test_res
        ~decode_pb_res:decode_pb_test_res
        () : (test_src_req, unary, test_res, unary) Client.rpc)
    open Pbrt_services
    
    let test_name : (test_name_req, unary, test_res, unary) Client.rpc =
      (Client.mk_rpc 
        ~package:["imandrax";"simple"]
        ~service_name:"Simple" ~rpc_name:"test_name"
        ~req_mode:Client.Unary
        ~res_mode:Client.Unary
        ~encode_json_req:encode_json_test_name_req
        ~encode_pb_req:encode_pb_test_name_req
        ~decode_json_res:decode_json_test_res
        ~decode_pb_res:decode_pb_test_res
        () : (test_name_req, unary, test_res, unary) Client.rpc)
    open Pbrt_services
    
    let qcheck_src : (qcheck_src_req, unary, test_res, unary) Client.rpc =
      (Client.mk_rpc 
        ~package:["imandrax";"simple"]
        ...
        ~decode_json_res:decode_json_test_res
        ~decode_pb_res:decode_pb_test_res
        () : (qcheck_src_req, unary, test_res, unary) Client.rpc)
    open Pbrt_services
    
    let qcheck_name : (qcheck_name_req, unary, test_res, unary) Client.rpc =
      (Client.mk_rpc 
        ~package:["imandrax";"simple"]
        ...
        ~decode_json_res:decode_json_test_res
        ~decode_pb_res:decode_pb_test_res
        () : (qcheck_name_req, unary, test_res, unary) Client.rpc)
    open Pbrt_services
    ...
    let get_decls : (get_decls_req, unary, get_decls_res, unary) Client.rpc =
      (Client.mk_rpc 
        ~package:["imandrax";"simple"]
        ~service_name:"Simple" ~rpc_name:"get_decls"
        ~req_mode:Client.Unary
        ~res_mode:Client.Unary
        ~encode_json_req:encode_json_get_decls_req
        ~encode_pb_req:encode_pb_get_decls_req
        ~decode_json_res:decode_json_get_decls_res
        ~decode_pb_res:decode_pb_get_decls_res
        () : (get_decls_req, unary, get_decls_res, unary) Client.rpc)
    open Pbrt_services
  ...
  module Server = struct
    ...
    let test_src : (test_src_req,unary,test_res,unary) Server.rpc = 
      (Server.mk_rpc ~name:"test_src"
        ~req_mode:Server.Unary
        ~res_mode:Server.Unary
        ~encode_json_res:encode_json_test_res
        ~encode_pb_res:encode_pb_test_res
        ~decode_json_req:decode_json_test_src_req
        ~decode_pb_req:decode_pb_test_src_req
        () : _ Server.rpc)
    
    let test_name : (test_name_req,unary,test_res,unary) Server.rpc = 
      (Server.mk_rpc ~name:"test_name"
        ~req_mode:Server.Unary
        ~res_mode:Server.Unary
        ~encode_json_res:encode_json_test_res
        ~encode_pb_res:encode_pb_test_res
        ~decode_json_req:decode_json_test_name_req
        ~decode_pb_req:decode_pb_test_name_req
        () : _ Server.rpc)
    
    let qcheck_src : (qcheck_src_req,unary,test_res,unary) Server.rpc = 
    ...
    let qcheck_name : (qcheck_name_req,unary,test_res,unary) Server.rpc = 
    ...
    let get_decls : (get_decls_req,unary,get_decls_res,unary) Server.rpc = 
      (Server.mk_rpc ~name:"get_decls"
        ~req_mode:Server.Unary
        ~res_mode:Server.Unary
        ~encode_json_res:encode_json_get_decls_res
        ~encode_pb_res:encode_pb_get_decls_res
        ~decode_json_req:decode_json_get_decls_req
        ~decode_pb_req:decode_pb_get_decls_req
        () : _ Server.rpc)
    ...
    let make
      ...
      ~test_src:__handler__test_src
      ~test_name:__handler__test_name
      ...
      ~get_decls:__handler__get_decls
      ...
      { Server.
        ...
        handlers=[
           ...
           (__handler__test_src test_src);
           (__handler__test_name test_name);
           ...
           (__handler__get_decls get_decls);
