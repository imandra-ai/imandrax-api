...
def Error_Error_core_message_of_twine(d: twine.Decoder, off: int) -> Error_Error_core_message:
    ...
    bt = twine.optional(d=d, off=fields[2], d0=lambda off: d.get_str(off=off))
...
def Ty_view_adt_row_of_twine[_V_tyreg_poly_id,_V_tyreg_poly_t](d: twine.Decoder, d0: Callable[...,_V_tyreg_poly_id],d1: Callable[...,_V_tyreg_poly_t],off: int) -> Ty_view_adt_row:
    ...
    labels = twine.optional(d=d, off=fields[1], d0=lambda off: [decode__tyreg_poly_id(d=d,off=x) for x in d.get_array(off=off)])
    args = [decode__tyreg_poly_t(d=d,off=x) for x in d.get_array(off=fields[2])]
    doc = twine.optional(d=d, off=fields[3], d0=lambda off: d.get_str(off=off))
...
def Ty_view_rec_row_of_twine[_V_tyreg_poly_id,_V_tyreg_poly_t](d: twine.Decoder, d0: Callable[...,_V_tyreg_poly_id],d1: Callable[...,_V_tyreg_poly_t],off: int) -> Ty_view_rec_row:
    ...
    doc = twine.optional(d=d, off=fields[2], d0=lambda off: d.get_str(off=off))
...
def Admission_of_twine(d: twine.Decoder, off: int) -> Admission:
    ...
    measure_fun = twine.optional(d=d, off=fields[1], d0=lambda off: Uid_of_twine(d=d, off=off))
...
def Model_fi_of_twine[_V_tyreg_poly_term,_V_tyreg_poly_var,_V_tyreg_poly_ty](d: twine.Decoder, d0: Callable[...,_V_tyreg_poly_term],d1: Callable[...,_V_tyreg_poly_var],d2: Callable[...,_V_tyreg_poly_ty],off: int) -> Model_fi:
    ...
    fi_cases = [([decode__tyreg_poly_term(d=d,off=x) for x in d.get_array(off=x)],decode__tyreg_poly_term(d=d,off=x)) for x in d.get_array(off=fields[2])]
...
def Model_of_twine[_V_tyreg_poly_term,_V_tyreg_poly_fn,_V_tyreg_poly_var,_V_tyreg_poly_ty](d: twine.Decoder, d0: Callable[...,_V_tyreg_poly_term],d1: Callable[...,_V_tyreg_poly_fn],d2: Callable[...,_V_tyreg_poly_var],d3: Callable[...,_V_tyreg_poly_ty],off: int) -> Model:
    ...
    fields = list(d.get_array(off=off))
    tys = [(decode__tyreg_poly_ty(d=d,off=x),Model_ty_def_of_twine(d=d,off=x,d0=(lambda off: decode__tyreg_poly_term(d=d,off=off)),d1=(lambda off: decode__tyreg_poly_ty(d=d,off=off)))) for x in d.get_array(off=fields[0])]
    consts = [(decode__tyreg_poly_fn(d=d,off=x),decode__tyreg_poly_term(d=d,off=x)) for x in d.get_array(off=fields[1])]
    funs = [(decode__tyreg_poly_fn(d=d,off=x),Model_fi_of_twine(d=d,off=x,d0=(lambda off: decode__tyreg_poly_term(d=d,off=off)),d1=(lambda off: decode__tyreg_poly_var(d=d,off=off)),d2=(lambda off: decode__tyreg_poly_ty(d=d,off=off)))) for x in d.get_array(off=fields[2])]
    representable = d.get_bool(off=fields[3])
    completed = d.get_bool(off=fields[4])
    ty_subst = [(Uid_of_twine(d=d, off=x),decode__tyreg_poly_ty(d=d,off=x)) for x in d.get_array(off=fields[5])]
...
def Ca_store_Ca_ptr_Raw_of_twine(d: twine.Decoder, off: int) -> Ca_store_Ca_ptr_Raw:
    fields = list(d.get_array(off=off))
    key = Ca_store_Key_of_twine(d=d, off=fields[0])
...
def Cir_Type_of_twine(d: twine.Decoder, off: int) -> Cir_Type:
    fields = list(d.get_array(off=off))
    view = Ty_view_view_of_twine(d=d,off=fields[0],d0=(lambda off: d.get_null(off=off)),d1=(lambda off: Cir_Type_var_of_twine(d=d, off=off)),d2=(lambda off: Cir_Type_of_twine(d=d, off=off)))
...
def Cir_Type_def_of_twine(d: twine.Decoder, off: int) -> Cir_Type_def:
    ...
    decl = Ty_view_decl_of_twine(d=d,off=fields[2],d0=(lambda off: Uid_of_twine(d=d, off=off)),d1=(lambda off: Cir_Type_of_twine(d=d, off=off)),d2=(lambda off: Void_of_twine(d=d, off=off)))
    clique = twine.optional(d=d, off=fields[3], d0=lambda off: Cir_Type_clique_of_twine(d=d, off=off))
    timeout = twine.optional(d=d, off=fields[4], d0=lambda off: d.get_int(off=off))
...
def Cir_Fo_pattern_of_twine(d: twine.Decoder, off: int) -> Cir_Fo_pattern:
    fields = list(d.get_array(off=off))
    view = Cir_Fo_pattern_view_of_twine(d=d,off=fields[0],d0=(lambda off: Cir_Fo_pattern_of_twine(d=d, off=off)))
    ty = Cir_Type_of_twine(d=d, off=fields[1])
...
def Cir_Case_of_twine[_V_tyreg_poly_t](d: twine.Decoder, d0: Callable[...,_V_tyreg_poly_t],off: int) -> Cir_Case:
    ...
    case_labels = twine.optional(d=d, off=fields[3], d0=lambda off: [Uid_of_twine(d=d, off=x) for x in d.get_array(off=off)])
...
def Cir_Logic_config_of_twine(d: twine.Decoder, off: int) -> Cir_Logic_config:
    ...
    max_induct = twine.optional(d=d, off=fields[3], d0=lambda off: d.get_int(off=off))
...
def Cir_Fun_def_of_twine(d: twine.Decoder, off: int) -> Cir_Fun_def:
    ...
    f_clique = twine.optional(d=d, off=fields[4], d0=lambda off: Cir_Clique_of_twine(d=d, off=off))
    f_kind = Cir_Fun_def_fun_kind_of_twine(d=d, off=fields[5])
    f_admission = twine.optional(d=d, off=fields[6], d0=lambda off: Admission_of_twine(d=d, off=off))
    f_admission_measure = twine.optional(d=d, off=fields[7], d0=lambda off: Uid_of_twine(d=d, off=off))
    f_validate_strat = Cir_Fun_def_validation_strategy_of_twine(d=d, off=fields[8])
    f_hints = twine.optional(d=d, off=fields[9], d0=lambda off: Cir_Hints_Top_of_twine(d=d,off=off,d0=(lambda off: Cir_Fun_def_apply_hint_of_twine(d=d, off=off))))
    ...
    f_timeout = twine.optional(d=d, off=fields[12], d0=lambda off: d.get_int(off=off))
...
def Cir_Fun_def_apply_hint_of_twine(d: twine.Decoder, off: int) -> Cir_Fun_def_apply_hint:
    fields = list(d.get_array(off=off))
    apply_fun = Cir_Fun_def_of_twine(d=d, off=fields[0])
...
def Cir_Rewrite_rule_of_twine(d: twine.Decoder, off: int) -> Cir_Rewrite_rule:
    ...
    rw_loop_break = twine.optional(d=d, off=fields[8], d0=lambda off: Cir_Fo_pattern_of_twine(d=d, off=off))
...
def Cir_Proof_obligation_of_twine(d: twine.Decoder, off: int) -> Cir_Proof_obligation:
    ...
    timeout = twine.optional(d=d, off=fields[5], d0=lambda off: d.get_int(off=off))
...
def Cir_Db_ser_of_twine(d: twine.Decoder, off: int) -> Cir_Db_ser:
    ...
    rw_rules = Cir_Db_ser_ph_map_of_twine(d=d,off=fields[1],d0=(lambda off: [Cir_Db_ser_ca_ptr_of_twine(d=d,off=x,d0=(lambda off: Cir_Rewrite_rule_of_twine(d=d, off=off))) for x in d.get_array(off=off)]))
    inst_rules = Cir_Db_ser_uid_map_of_twine(d=d,off=fields[2],d0=(lambda off: Cir_Db_ser_ca_ptr_of_twine(d=d,off=off,d0=(lambda off: Cir_Instantiation_rule_of_twine(d=d, off=off)))))
    rule_spec_fc = Cir_Db_ser_uid_map_of_twine(d=d,off=fields[3],d0=(lambda off: [Cir_Db_ser_ca_ptr_of_twine(d=d,off=x,d0=(lambda off: Cir_Trigger_of_twine(d=d, off=off))) for x in d.get_array(off=off)]))
    rule_spec_rw_rules = Cir_Db_ser_uid_map_of_twine(d=d,off=fields[4],d0=(lambda off: [Cir_Db_ser_ca_ptr_of_twine(d=d,off=x,d0=(lambda off: Cir_Rewrite_rule_of_twine(d=d, off=off))) for x in d.get_array(off=off)]))
    fc = Cir_Db_ser_ph_map_of_twine(d=d,off=fields[5],d0=(lambda off: [Cir_Db_ser_ca_ptr_of_twine(d=d,off=x,d0=(lambda off: Cir_Trigger_of_twine(d=d, off=off))) for x in d.get_array(off=off)]))
    elim = Cir_Db_ser_ph_map_of_twine(d=d,off=fields[6],d0=(lambda off: [Cir_Db_ser_ca_ptr_of_twine(d=d,off=x,d0=(lambda off: Cir_Elimination_rule_of_twine(d=d, off=off))) for x in d.get_array(off=off)]))
    gen = Cir_Db_ser_ph_map_of_twine(d=d,off=fields[7],d0=(lambda off: [Cir_Db_ser_ca_ptr_of_twine(d=d,off=x,d0=(lambda off: Cir_Trigger_of_twine(d=d, off=off))) for x in d.get_array(off=off)]))
    thm_as_rw = Cir_Db_ser_uid_map_of_twine(d=d,off=fields[8],d0=(lambda off: [Cir_Db_ser_ca_ptr_of_twine(d=d,off=x,d0=(lambda off: Cir_Rewrite_rule_of_twine(d=d, off=off))) for x in d.get_array(off=off)]))
    thm_as_fc = Cir_Db_ser_uid_map_of_twine(d=d,off=fields[9],d0=(lambda off: [Cir_Db_ser_ca_ptr_of_twine(d=d,off=x,d0=(lambda off: Cir_Instantiation_rule_of_twine(d=d, off=off))) for x in d.get_array(off=off)]))
    thm_as_elim = Cir_Db_ser_uid_map_of_twine(d=d,off=fields[10],d0=(lambda off: [Cir_Db_ser_ca_ptr_of_twine(d=d,off=x,d0=(lambda off: Cir_Elimination_rule_of_twine(d=d, off=off))) for x in d.get_array(off=off)]))
    thm_as_gen = Cir_Db_ser_uid_map_of_twine(d=d,off=fields[11],d0=(lambda off: [Cir_Db_ser_ca_ptr_of_twine(d=d,off=x,d0=(lambda off: Cir_Instantiation_rule_of_twine(d=d, off=off))) for x in d.get_array(off=off)]))
    admission = Cir_Db_ser_uid_map_of_twine(d=d,off=fields[12],d0=(lambda off: Cir_Db_ser_ca_ptr_of_twine(d=d,off=off,d0=(lambda off: Admission_of_twine(d=d, off=off)))))
    count_funs_of_ty = Cir_Db_ser_uid_map_of_twine(d=d,off=fields[13],d0=(lambda off: Uid_of_twine(d=d, off=off)))
    config = Cir_Db_ser_ca_ptr_of_twine(d=d,off=fields[14],d0=(lambda off: Cir_Logic_config_of_twine(d=d, off=off)))
...
def Eval_Value_of_twine(d: twine.Decoder, off: int) -> Eval_Value:
    fields = list(d.get_array(off=off))
    v = Eval_Value_view_of_twine(d=d,off=fields[0],d0=(lambda off: Eval_Value_of_twine(d=d, off=off)),d1=(lambda off: d.get_null(off=off)))
...
def Report_Smt_proof_of_twine[_V_tyreg_poly_term](d: twine.Decoder, d0: Callable[...,_V_tyreg_poly_term],off: int) -> Report_Smt_proof:
    ...
    expansions = [Report_Expansion_of_twine(d=d,off=x,d0=(lambda off: decode__tyreg_poly_term(d=d,off=off))) for x in d.get_array(off=fields[2])]
    instantiations = [Report_Instantiation_of_twine(d=d,off=x,d0=(lambda off: decode__tyreg_poly_term(d=d,off=off))) for x in d.get_array(off=fields[3])]
...
def Report_Report_of_twine(d: twine.Decoder, off: int) -> Report_Report:
    fields = list(d.get_array(off=off))
    events = [Report_Report_event_of_twine(d=d, off=x) for x in d.get_array(off=fields[0])]
...
def Proof_Proof_term_poly_of_twine[_V_tyreg_poly_term,_V_tyreg_poly_ty,_V_tyreg_poly_proof](d: twine.Decoder, d0: Callable[...,_V_tyreg_poly_term],d1: Callable[...,_V_tyreg_poly_ty],d2: Callable[...,_V_tyreg_poly_proof],off: int) -> Proof_Proof_term_poly:
    ...
    concl = Sequent_poly_of_twine(d=d,off=fields[1],d0=(lambda off: decode__tyreg_poly_term(d=d,off=off)))
    view = Proof_View_of_twine(d=d,off=fields[2],d0=(lambda off: decode__tyreg_poly_term(d=d,off=off)),d1=(lambda off: decode__tyreg_poly_ty(d=d,off=off)),d2=(lambda off: decode__tyreg_poly_proof(d=d,off=off)))
...
def Proof_Cir_proof_term_of_twine(d: twine.Decoder, off: int) -> Proof_Cir_proof_term:
    fields = list(d.get_array(off=off))
    p = Proof_Cir_proof_term_t_inner_of_twine(d=d, off=fields[0])
...
def Tasks_PO_res_no_proof_of_twine(d: twine.Decoder, off: int) -> Tasks_PO_res_no_proof:
    ...
    counter_model = twine.optional(d=d, off=fields[1], d0=lambda off: Cir_Model_of_twine(d=d, off=off))
...
def Tasks_PO_res_of_twine(d: twine.Decoder, off: int) -> Tasks_PO_res:
    fields = list(d.get_array(off=off))
    from_ = Ca_store_Ca_ptr_of_twine(d=d,off=fields[0],d0=(lambda off: Cir_Proof_obligation_of_twine(d=d, off=off)))
    res = Tasks_PO_res_result_of_twine(d=d,off=fields[1],d0=(lambda off: Tasks_PO_res_success_of_twine(d=d, off=off)))
    stats = Tasks_PO_res_stats_of_twine(d=d, off=fields[2])
    report = In_mem_archive_of_twine(d=d,off=fields[3],d0=(lambda off: Report_Report_of_twine(d=d, off=off)))
...
def Tasks_Eval_task_of_twine(d: twine.Decoder, off: int) -> Tasks_Eval_task:
    ...
    timeout = twine.optional(d=d, off=fields[3], d0=lambda off: d.get_int(off=off))
...
def Tasks_Eval_res_success_of_twine(d: twine.Decoder, off: int) -> Tasks_Eval_res_success:
    fields = list(d.get_array(off=off))
    v = Tasks_Eval_res_value_of_twine(d=d, off=fields[0])
...
def Tasks_Eval_res_of_twine(d: twine.Decoder, off: int) -> Tasks_Eval_res:
    fields = list(d.get_array(off=off))
    res = twine_result(d=d, off=fields[0], d0=lambda off: Tasks_Eval_res_success_of_twine(d=d, off=off), d1=lambda off: Error_Error_core_of_twine(d=d, off=off))
